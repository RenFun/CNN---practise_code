...
for i in range(EPOCH):
    ...
    for data in train_loader:
        ...
        img = img.to(DEVICE)
        label = label.to(DEVICE)
        # 将label进行one-hot编码，规模变为[128, 10]，并变成float类型
        label = functional.one_hot(label, num_classes=10)
        ...
        # 反向传播误差，但是参数还没更新
        loss.backward()
        # 如果出现数据类型错误，将loss.backward(torch.ones_like(loss))
        # 更新模型参数
        optimizer.step()
    train_loss.append(x/BATCH_SIZE)
    print('训练损失值：', x/BATCH_SIZE)
    ...
    for data in test_loader:
        ...
        # test_loss.append(loss.data.item())
        _, prediction = torch.max(out, 1)
        # 将onr-hot编码再变回标签形式，torch.argmax(dim)会返回dim维度上张量最大值的索引
