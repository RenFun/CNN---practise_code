...
# 选择Adam为优化器
optimizer2 = torch.optim.Adam(cnn.parameters(), lr=LR)
optimizer3 = torch.optim.Adagrad(cnn.parameters(), lr=LR)
# 加载训练集和测试集
...
plt.subplot(121)
plt.plot(np.arange(1, 21), train_loss[0: 20], color='lightblue', label='SGD')
plt.plot(np.arange(1, 21), train_loss[20: 40], color='royalblue', label='Adam')
plt.plot(np.arange(1, 21), train_loss[40: 60], color='orange', label='Adagrad')
plt.xlabel("迭代次数")
plt.ylabel("训练损失值")
...
# 绘制图像2：测试集上的精度
plt.subplot(122)
plt.plot(np.arange(1, 21), accuracy[0: 20], color='lightblue', label='SGD')
plt.plot(np.arange(1, 21), accuracy[20: 40], color='royalblue', label='Adam')
plt.plot(np.arange(1, 21), accuracy[40: 60], color='orange', label='Adagrad')
plt.xlabel("迭代次数")
plt.ylabel("精度")
plt.xticks(np.arange(1, 21, 1))
plt.grid(b=True, linestyle='--')
...
plt.savefig('CNN_mnist_optimizer_loss&accuracy.svg', bbox_inches='tight')
=======
# Auther:RenFun
# File:CNN_mnist_optimizer.py
# Time:2022/01/13
